...
A logarithmic wind speed profile was fit to observed average wind speeds over the March 13-14 snowfall event from three anemometors installed at 3, 4, and 15 m above ground at FT station.

The hydrometeor trajectories over this event were not directly observed but could be estimated following @eq-ta using observed wind speed and mean hydrometeor terminal velocity observed over the event of 0.9 m s^-1^. The wind speed observed over the event followed a logarithmic wind speed profile with height above the snowpack. @fig-wind-profiles shows a higher hydrometeor trajectory angle was predicted for the top of the canopy as a result of the higher wind speed, for example at 10 m a trajectory angle of `r ft_10m_example$traj_angle |> round(2)`° was calculated, compared to a trajectory angle of `r ft_2m_example$traj_angle |> round(2)`° at 2 m above the snowpack. which has higher wind speed, compared to lower levels of the canopy which were observed to have significantly lower wind speeds. The average wind speed observed over the wind profile, calculated as an integral between the displacement height plus roughness hight to the average height of the FT plot trees was `ft_mean_wind_prof_integral$wind_speed |> round(2)` m s^-1^ and the corresponding trajectory angle resulting from this was `ft_mean_wind_prof_integral$traj_angle |> round(2)`°. The trajectory angle resulting from this average wind speed is much higher than the trajectory angle closer to 30° observed in @fig-hemi-ip-cc. The trajectory angles of around 30° would have resulted from a mean wind speed of around 0.5 m s^-1^ and a height above the snowpack of about 1.5 m. This lower height is interpreted to be a result of the conical shape of the needleleaf trees surrounding PWL and FT which have the majority of their neeleleaf elements lower towards the ground. Therefore while the wind speeds were observed to be higher near the top of the canopy, corresponding to higher trajectory angles, the reduced amount of needleleaf elements at this height result in a smaller inpact of these more horizontal trajectories. 
...
Now need to show the increase in C_p at the plot scale and how this is a better predictor of I/P compared to nadir. Then in the following section we will build up how to calculate the increase in plot scale I/P.
...
NEED TO REORDER THOUGHT PROCESS HERE:

Goal is to predict the plot scale increase in canopy coverage based on wind speed. 

  1. Show obs:
  
  ![Observed leaf contact area using VoxRS.](../../analysis/lidar-processing/figs/voxrs/scatter/WITH_TREEWELLS_traj_angle_and_wind_phiby_1_thetaby_2_traj_angle_wind_speed_vs_fractional_inc_lca_mean_sd_23_072.png){#fig-finclca-ht-ws}
  2. Can calculate lca based on the nadir lca + the increase in lca from nadir
  
  
@fig-finclca-ht-ws shows the observed increase in $C_p$, prior to snowfall on March 13th, for trajectory angles [0°, 1°, ..., 90°] and wind speeds [0 – 10 (m s^-1^)]. Increasing the hydrometeor trajectory angle results in a substantial rise in the observed $C_p$ over relatively common wind speeds. For example, with a wind speed of `r select_wind_speed` m s^-1^ which leads to a trajectory angle of `r ta |> round(0)`° using @eq-ta, the $C_p$ increased by a fraction of `r pwl_inc |> round(2)` and `r ft_inc |> round(2)` for the PWL and FT forest plots respectively in @fig-lca-ht-ws (right panel). This is a percent increase in the plot $C_p$ from nadir of `r pwl_perc_inc |> round(0)`% and `r ft_perc_inc |> round(0)`% for PWL and FT respectively. $C_p$ was also quantified across trajectory angles for both PWL and FT on March 14th, post snowfall, and showed a negligible effect of canopy snow load on $C_p$.
...
@fig-lca-ht-ws shows the simulated $C_p$ aligns well with the VoxRS observations for trajectory angles below 60°. Model error statistics shown in @tbl-lca-mod-err, indicate that @eq-lca-ac demonstrated slightly better performance with a mean bias and RMSE of -0.005 and 0.033 respectively, compared to the linear regression, with a mean bias of 0.006 and an RMSE of 0.044. The @Hedstrom1998a method exhibited very poor estimates of $C_p$.
...
The ultimate variable we are after is throughfall so need to show the performance of throughfal models. We can do this at the plot/coarse grid resolution where the vector based model works. We can also ignore the vector based soluation and just use the cc_nadir to predict I/P at a smaller grid resolution... if we want a detailed map of throughfall.
