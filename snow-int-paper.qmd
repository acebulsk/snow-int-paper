...
$$
\frac{dL}{dt} = 
q_{sf} - q_{tf} - q_{unld}(L) - q_{drip}(L) - q_{wind}^{veg}(L) - q_{sub}^{veg}(L)
$$ {#eq-canopy-mass-bal}
...
#| label: tbl-lca-coefs
#| tbl-cap: "Coefficients derived from the non-linear least squares regression fit of Equation @eq-lca-inc to the VoxRS dataset."
...
![UAV-liar and in-situ snow survey measurements over the March 13–14th snowfall event and associated error metrics.](/home/alex/local-usask/analysis/lidar-processing/figs/lidar_snow_depth_analysis/pre_post_figs/23_073_v2.0.0_sa_snow_depth_Hs_insitu_vs_Hs_lidar_resamp_bias_corrected.png){#fig-lidar-vs-fsd-sd}
