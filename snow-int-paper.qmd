...
::: {#fig-lidar-ip layout="[[1, 1], [1,1]]"}

![PWL Canopy Height](../../analysis/lidar-processing/figs/maps/pwl_e_canopy_height_lidR.png){#fig-lidar-ch-pwl}

![FT Canopy Height](../../analysis/lidar-processing/figs/maps/frs_s_canopy_height_lidR.png){#fig-lidar-ch-ft}

![FT I/P](../../analysis/lidar-processing/figs/maps/pwl_e_23_072_23_073_v2.0.0_saip_normalised_resample_0.25.png){#fig-lidar-ip-pwle}
...
Canopy height (top row) and interception efficiency calculated over a 24 hr snowfall event from March 13, 2023 to March 14, 2024 (bottom row) for the PWL forest plot (left column) and FT forest plot (right column) at a 25 cm resolution. The canopy height for PWL (a), FT (b). Interception efficiency for PWL (c) and FT (d). White areas for the bottom row are 25 cm grids that did not have any lidar ground returns or have been masked due to disturbance and thus, no throughfall measurement for the I/P calculation.
...
Scatter plots showing the association of (a) hydrometeor trajectory angle and wind speed with mean contact number and (b) hydrometeor trajectory angle and wind speed with apparent canopy coverage calculated as a function of mean contact number. The dots represent the mean mean contact number (a) OR mean canopy coverage (b) across all azimuth angles of the hemisphere [0, 1, ..., 359] for a each zenith angle [0, 1, ..., 90] at each forest plot. The colour of the dot represents the mean canopy coverage of each forest plot from nadir. Trajectory angle was estimated as a function of wind speed.
...
-   Forest structure is the main factor governing the fraction of intercepted snowfall at a particular site, with meteorological conditions contributing less to variability related to initial interception.
...
-   Interception efficiency was shown to increase with wind speed and canopy snow load as a result of increasing the snow-leaf contact area ratio.
-   High wind speeds were observed to decrease intercepted load due to increased snow unloading.
