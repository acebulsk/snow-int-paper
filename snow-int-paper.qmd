...
-   @Hedstrom1998 include interception and unloading in one equation as discussed in @Cebulski2024b. The purpose of this study is to develop a new snow interception parameterization that does not include unloading as suggested by @Cebulski2024b
...
where $\overline{q_{sf}}$ and $\overline{q_{tf}}$ are the average snowfall and throughfall rate over $\Delta t$. $\Delta SWE_o$ is the change in SWE to the open (kg m^-2^).
...
The observations presented in this study showing the minimal influence of meteorology or snow load on interception efficiency observed in @fig-lai-met-ip, the relatively consistent increase in canopy snow load with snowfall in @fig-scl-w-sf, minimal influence of snow load on $C_p$ observed by VoxRS and strong linear relationship found between $C_p$ and interception efficiency suggests existing snow interception parameterizations may need to be revised. The absence of influence of air temperature on interception efficiency or canopy snow load on interception efficiency or $C_p$ observed in this study, which form the basis of existing interception parameterizations [@Hedstrom1998a; @Storck2002], may be a result of the measurements used to develop existing theories. The measurements of interception efficiency in previous studies might have been influenced by some downward ablation, as discussed in @Cebulski2024b. Given the findings presented in this study and suggestions in @Cebulski2024b to separate of ablative processes from interception parameterizations provide impetus to restructure snow interception as a function of $C_p$. This new snow interception parameterization is consistent with rainfall interception literature, which calculate throughfall as a function of $C_c$ [i.e., @Valante1997], and minimizes the mixing of unloading and interception processes. This also provides a method to incorporate more commonly available canopy metrics such as $C_c$ which are subject to less measurement bias compared to leaf area index, which is currently used by existing interception parameterizations [@Hedstrom1998a; @Storck2002]. However, the large sensitivity of leaf contact area to hydrometeor trajectory angle, resulting from relatively common wind speeds of 1-2 m s^-1^ suggests that this may be an important process to consider in snow dominated basins. 

The plot scale canopy coverage was shown to increase substantially as a result of increasing horizontal angle of hydrometers. The increase in plot scale canopy coverage observed in this study, of from 0.4 with zero wind to near complete coverage with midcanopy wind speeds of 2 m s^-1^ is much higher than the increase calculated using the method proposed in @Hedstrom1998a to scale canopy coverage with wind speed. The increase in interception efficiency observed away from individual trees in @fig-lidar-ip and the measured increase plot scale canopy coverage using VoxRS in @fig-lca-ht-ws show that Equation 10 in @Hedstrom1998a may not be appropriate for sparse forests. The minimal influence of canopy snow load on leaf contact area observed at the plot scale after a 30 mm snowfall event suggests incorporating canopy snow load in interception efficiency parameterizations but may be an important process to consider for canopy snow unloading.
