...
Measurements of interception efficiency conducted over two winter seasons show that for low wind speed events leaf area index (VZA of 45°) is strongly associated (R^2^ = 0.68, p < 0.001) with interception efficiency (@fig-lai-ip). For higher wind speeds (> 1.3 m s^-1^), @fig-lai-ip shows that the association between leaf area index and interception efficiency is reduced (R^2^ = 0.22, p < 0.001). Wider zenith angles, which incorporates more information about the forest surrounding the snow survey station, were found to have a stronger relationship with interception efficiency compared to narrower zenith angles of 15 degrees. Snowfall events with some wind lead to non-vertical hydrometeor trajectories where the surrounding forest structure is expected to be important in controlling snowpack accumulation to the point measurement. The event median wind speed was used when classifying the snowfall events to better reflect the sustained wind speed over the event rather than the mean which is influenced by gusts. For the high wind speed events, an increase in interception efficiency is observed at locations with low LAI while reducing interception efficiency in locations with higher LAI and CC due to wind induced unloading. Since snow survey data was used here, which had longer event intervals (@tbl-ss-met), some unloading and wind redistribution may have also affected these results.
...
![Scatterplot of snow survey interception efficiency based on snow surveys plotted against leaf area index of each station for six snowfall events with low (purple, 0.69-1.27 m/s) and high (orange, 1.37-1.72 m/s) median wind speed. Leaf area index shown here was measured using hemispherical photography analysis with a vertical zenith angle of 45°. The black open circles show the mean interception efficiency of each leaf area index bin and the error bars represent the standard deviations.](figs/snow_survey_periods/lai_vs_ip.png){#fig-lai-ip}

![Snow survey interception efficiency plotted against meterological conditions. The black open circles show the mean of each bin and the error bars represent the standard deviations for each of the six snow surveys.](figs/snow_survey_periods/snow_survey_ip_w_met.png){#fig-ss-met}
