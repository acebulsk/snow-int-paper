...
#| tbl-cap: "Results of the Wilcoxon signed-rank tests comparing the distributions of hourly interception efficiency (IP) measured by the SCLs for differing groups of air temperatures (Ta), wind speeds (u), and initial canopy snow loads (L). The table reports the canopy corresponding to the SCL (Canopy), null hypothesis ($H_0$), $p$-value, and sample size ($n$) and median IP for for the 'low' group (e.g., Ta < -5°C) and 'high' group (e.g., Ta ≥ -5°C)."
